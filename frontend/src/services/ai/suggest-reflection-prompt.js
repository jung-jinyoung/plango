...
function buildPrompt({ dailyIntent, schedules }) {
  ...
  if (total === 0) {
    return dailyIntent
      ? `오늘 계획했던 "${dailyIntent}"를 향해 어떤 하루를 보내셨나요?`
  ...
  if (rate >= 80) return `오늘 ${total}개 중 ${done}개를 완료했어요. 특히 잘 풀린 일은 무엇이었나요?`
  if (rate <= 30) return '오늘은 계획대로 잘 안 풀린 하루였네요. 무엇이 발목을 잡았는지 적어보세요.'
  return `오늘 ${done}/${total}개를 완료했어요. 다음엔 어떻게 하면 더 나아질지 생각해보세요.`
}
